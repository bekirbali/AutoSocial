...
export const KanbanView: React.FC<KanbanViewProps> = ({ articles, onRefresh }) => {
  ...
  const handleAction = async (
    id: string,
    action: 'publish_now' | 'schedule' | 'reject',
  ) => {
    try {
      setIsActing(true);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ─── 3 Sütunlu Kanban Tahtası ──────────────────────────────────────── */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        {/* SÜTUN 1: ONAY BEKLEYENLER */}
        <div className="flex flex-col h-full bg-slate-900/40 border border-slate-800/80 rounded-2xl p-4">
          ...
          <div className="space-y-3.5 overflow-y-auto max-h-[calc(100vh-280px)] pr-1">
            {pendingArticles.length === 0 ? (
              ...
              pendingArticles.map((article) => (
                <div
                  key={article.id}
                  className="glass-card rounded-xl p-3.5 space-y-3 border-slate-800/80 hover:border-amber-500/30 transition group"
                >
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${getCategoryColor(article.category)}`}>
                  ...
                  {article.cardImageUrl ? (
                    ...
                    <div className="p-3 bg-slate-950 rounded-lg text-xs text-slate-500 text-center border border-dashed border-slate-800">
                      Görsel Şablon
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-1.5 pt-1 border-t border-slate-800/60">
                    ...
                    <button
                      ...
                      disabled={isActing}
                      className="flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-lg bg-slate-800 hover:bg-rose-900/40 text-slate-400 hover:text-rose-300 text-xs font-medium transition cursor-pointer"
                    >
                      <X className="w-3 h-3" />
                      <span>Pas Geç</span>
                    </button>
